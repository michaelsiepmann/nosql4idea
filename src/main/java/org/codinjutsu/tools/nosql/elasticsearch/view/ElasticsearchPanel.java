...
public class ElasticsearchPanel extends DatabasePanel<ElasticsearchClient, ElasticsearchContext, ElasticsearchResult, JsonObject> {
    ...
    protected ElasticsearchResult getSearchResult(ElasticsearchContext context, QueryOptions queryOptions) {
        return context.getClient().loadRecords(context.getServerConfiguration(), context.getElasticsearchDatabase(), new ElasticsearchQuery(queryOptions.getResultLimit(), context.getElasticsearchCollection()));
    }

    @Override
    public Object getRecords() {
        return getContext().getElasticsearchCollection();
    }
}
