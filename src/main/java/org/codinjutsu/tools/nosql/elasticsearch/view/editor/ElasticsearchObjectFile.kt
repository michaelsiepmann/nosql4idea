...
internal class ElasticsearchObjectFile(project: Project, configuration: ElasticsearchServerConfiguration, val database: ElasticsearchDatabase, val collection: ElasticsearchCollection?) :
        NoSqlDatabaseObjectFile<ElasticsearchServerConfiguration>(project, configuration, String.format("%s/%s/%s", configuration.label, database.name, collection?.name)) {
